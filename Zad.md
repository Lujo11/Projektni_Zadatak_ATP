...
znakova se naziva "praznim nizom" (eng. *empty string*) i na neki
način odgovara konceptu nule kod brojeva.

U Pythonu se tekst čuva kroz vrstu podataka 'str'{.python}, što je
skraćeno od engleske riječi *string* odnosno "niza". Svaka vrijednost
...
''' python
tekst1 = "ovo je u navodnicima pa se smatra tekstom"
tekst2 = 'ovo je u apostrofima pa se isto smatra tekstom'
tekst3 = "ako je ' u tekstu moramo koristiti navodnike"
tekst4 = 'ako je " u tekstu koristi apostrofe'
tekst5 = "ovaj \"tekst\" je validno zapisan"
...
Svaki niz znakova u Python kodu, kao i općenito u računalnim
...
eksperimentirali s vlastitim kodom. Kako jezik prijavljuje pogreške i
...
ima znatno više no u ovom primjeru im služe u pronalaženju gdje se greška
...
tekst[0:tekst.find('y')]  # dohvati od prvog znaka pa do prvog pojavljivanja 'y' -> 'Mont'
'''
...
organizacija koda. Na ovaj način, metoda 'upper'{.python} je svrstana
...
''' python
tekst = 'Ovaj tekst sadrži prijelome redaka'
...
''' python
tekst = ' Bi sam i'
print(tekst.format('o', 'vani', 'ludo sam se zabavio!'))  # Bio sam vani i ludo sam se zabavio!

tekst = 'la sam na predavanju i '
print(tekst.format('Bila', 'na predavanju', 'bilo mi je dosadno :( !'))  # Bila sam na predavanju i bilo mi je dosadno :( !
'''
...
ubacio neki tekst. U prethodnom primjeru, koristimo samo dva
...
tekst.replace('a', 'x')  # 'ne dx mi se ovo čitxt'

# može i s duljim nizovima
tekst.replace('ne da mi se', 'želim')  # 'želim ovo čitati'
